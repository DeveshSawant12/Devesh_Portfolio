...
export const MobileMenu: React.FC<{ navItems: { name: string; href: string }[] }> = ({ navItems }) => {
  ...
  return (
    <ul
      className={clsx(
        "mobile-menu-container flex flex-col gap-3 rounded-b-lg border-gray-200 px-4 transition-all duration-300 ease-in-out md:hidden"
      )}
    >
      ...
      {/* Contact */}
      <li
        className="mobile-menu-item transform pt-3 transition-all duration-300 ease-in-out"
        style={{ "--index": navItems.length + 1 } as React.CSSProperties}
      >
        <Link
